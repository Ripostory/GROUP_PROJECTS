...
{	    
    ...
    vec3 irradiance = textureCube(skybox, N).rgb;
    vec3 diffuse = irradiance * albedo;
    vec3 specular = textureCube(skybox, reflect(-V, N)).rgb * F;
    vec3 finalColor = finalDiff * diffuse;
   ...
   frag_color = vec4(0.0f);
    ...
    //frag_color = vec4(finalDiff * albedo / 3.1415 + specular, 1.0f);
}
